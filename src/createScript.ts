...
export const cmd = (() => {
	...
	return Object.assign(
		function cmd(command: string, options?: ExecSyncOptions) {
			console.log(chalk.gray(`[${formatDate(new Date(), 'HH:mm:ss')}]`), chalk.bold('run cmd:'), chalk.green(command));
...
export const style = {
	header: (title: string) => chalk.bgBlue.white.bold(` ⬥ ${title}`.padEnd(process.stdout.columns - groupLevel * 2)),
	bool: (value: any, label: string) =>
		isEmpty(value) ? '' : style.label(label, chalk[value ? 'green' : 'red'](value ? '✔' : '✘')),
	number: (value: any, label: string) => (isEmpty(value) ? '' : style.label(label, chalk.yellow(_.round(value, 2)))),
	label: (label: any, value: any) =>
		isEmpty(value)
			? ''
			: chalk.bold(label + ': ') + (value && typeof value === 'object' ? trunc(30, JSON.stringify(value)) : value),
};
