...
const { args } = parseArgv({
    options: {
        ...
        'print-dependencies': { type: 'boolean', default: false },
    },
});

void createScript(async function init() {
    const ifReact = (list: TemplateStringsArray, ...rest: any[]) =>
        args.react ? '\n' + textBlock(list, ...rest) : '';
    ...
    if (args['print-dependencies']) {
        console.log(
            Array.from(
                new Set(result.match(/from\s+'([^']+)'/g)!.map(match => match.split("'")[1]).sort())
            ).filter(x => !x.startsWith('node:')).join(' ')
        )
    } else {
        console.log(result);
    }
